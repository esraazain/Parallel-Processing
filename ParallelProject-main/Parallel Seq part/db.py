...
def db_connect():
    return sqlite3.connect('newdata')
...
def select_All_from_Data1(conn):
    with conn:
        return conn.execute(select_Data1).fetchone()

def select_All_from_Data2(conn):
    with conn:
        return conn.execute(select_Data2).fetchone()

def select_All_from_Data3(conn):
    with conn:
        return conn.execute(select_Data3).fetchone()

def select_All_from_Data4(conn):
    with conn:
        return conn.execute(select_Data4).fetchone()

def select_All_from_Data5(conn):
    with conn:
        return conn.execute(select_Data5).fetchone()
